...
var daemon = function(config) {
    ...
    Object.defineProperties(this, {
        ...
        _xml: {
            ...
            get: function() {
                return require('./winsw').generateXml({
                    name: this.name,
                    id: this._exe,
                    script: '"' + wrapper + '" -f "' + this.script + '" -l "' + this.name + '" -g ' + this.grow + ' -w ' + this.wait + (this.maxRetries !== null ? ' -m ' + this.maxRetries : '') + ' -r ' + this.maxRestarts + ' -a ' + (this.abortOnError == true ? 'y' : 'n') + (config.cwd ? ' -d "' + config.cwd + '"' : ''),
        ...
        uninstallAtLocation: {
            enumerable: true,
            writable: false,
            value: function(waitTime, location) {
                var me = this;
                waitTime = waitTime || 2; // The same wait time as in the install method
                ...
                var uninstaller = function() {
                    // Uninstall the process
                    location += '\\daemon';
                    console.log('"' + location + '" uninstall');
                    me.execute('"' + path.resolve(location, me._exe) + '" uninstall', function(error, stdout, stderr) {
                        if (error) {
                            me.checkPermError(error);
                        } else if (stderr.trim().length > 0) {
                            console.log('Error: ', stderr);
