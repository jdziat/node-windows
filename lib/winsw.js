module.exports = {
    ...
    createExe: function(name, dir, callback) {
        ...
        dir = dir || process.cwd();
        //Write the Exe to the destination.
        ...
        var origin = p.join(__dirname, '..', 'bin', 'winsw', 'x' + (require('os').arch().indexOf('64') > 0 ? '64' : '86'), 'winsw.exe.config'),
            dest = p.join(dir, name.replace(/[^\w]/gi, '').toLowerCase() + '.exe.config'),
            data = fs.readFileSync(origin, { encoding: 'utf8' });
